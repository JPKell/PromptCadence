...
6. **Advance contract.** A step completes only on a declared `finish_reason` of `STOP` (or a
   ...
   attempt and rendered nowhere — the gap `docs/history/handoffs/D2_HANDOFF.md` §2 named); the job document also
...
| connection refused / DNS | Trajectory fails at T13 with the cause, after cancelling any job the request started (there is no `waiting` state — see the closing paragraph); health degraded | `LOADCOACH_UNAVAILABLE` |
